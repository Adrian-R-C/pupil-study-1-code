...
addOptional(p, 'withinlinesTwoGroup',0); %ajout adrian
addOptional(p, 'valMax',4); %ajout adrian
...
for g = 1:num_groups 
    ...
    for k = 1:num_locs
        ...
        if confs.withinlinesTwoGroup==1
            if ismember(k, [1:confs.valMax/2])
                scdataGroup1{g}(:,:,k) = [xdata, data_vals];
            elseif ismember(k, [confs.valMax/2+1:confs.valMax])
                scdataGroup2{g}(:,:,k) = [xdata, data_vals];
            end
        end
    ...
    if confs.withinlines==1
        for s = 1:size(scdata{g},1)                
            h.wl(g) = plot(squeeze(scdata{g}(s,1,3:4)),...
                squeeze(scdata{g}(s,2,3:4)),'color', [0.8 0.8 0.8]);
        ...
        %deux groupes : adrian
        if confs.withinlinesTwoGroup==1
            for s = 1:size(scdataGroup1{g},1)                
                h.wl(g) = plot(squeeze(scdataGroup1{g}(s,1,1:confs.valMax/2)),...
                    squeeze(scdataGroup1{g}(s,2,1:confs.valMax/2)),'color', [0.8 0.8 0.8]);
                uistack(h.wl(g),'bottom')
            end

            for s = 1:size(scdataGroup2{g},1)                
                h.wl(g) = plot(squeeze(scdataGroup2{g}(s,1,confs.valMax/2+1:confs.valMax)),...
                    squeeze(scdataGroup2{g}(s,2,confs.valMax/2+1:confs.valMax)),'color', [0.8 0.8 0.8]);
                uistack(h.wl(g),'bottom')
            end
        end
