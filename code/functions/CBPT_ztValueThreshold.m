function [uncorrectedSignificance correctedSignificance clusterCharacteristics] = CBPT_ztValueThreshold(matrix1, matrix2, pThreshold, nPermutation, dependentOrIndependent, tail, parametricOrNonParametric, alphaThreshold)
...
if string(tail) == "both" 
     tThreshold = tinv(1 - pThreshold/2, degreeOfFreedom);
     zThreshold = norminv(1 - pThreshold/2);
...
%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    maxMassClustersPositivePARAMETRIC=zeros(1,nPermutation);
    maxMassClustersNegativePARAMETRIC=maxMassClustersPositivePARAMETRIC;
    ...
    maxMassClustersPositiveNONPARAMETRIC=zeros(1,nPermutation);
    maxMassClustersNegativeNONPARAMETRIC=maxMassClustersPositiveNONPARAMETRIC;
...
if string(tail) == "both"
    ...
    %         numberOfPositiveClusterFoundBiggerThanInitialOnePARAMETRIC(cptInitialClusterPositive)=sum( maxSizeClustersPARAMETRIC > positiveClusterInitialPARAMETRIC.length(cptInitialClusterPositive) );
            ...
            if pValueFDRPositive(cptInitialClusterPositive) <= alphaThreshold
    ...
    %         numberOfNegativeClusterFoundBiggerThanInitialOnePARAMETRIC(cptInitialClusterNegative)=sum( maxSizeClustersPARAMETRIC > negativeClusterInitialPARAMETRIC.length(cptInitialClusterNegative) );
            ...
            if pValueFDRNegative(cptInitialClusterNegative) <= alphaThreshold
