function [tValue pValuePARAMETRIC zValue pValueNONPARAMETRIC positiveClusterPARAMETRIC negativeClusterPARAMETRIC positiveClusterNONPARAMETRIC negativeClusterNONPARAMETRIC] = extractClusterFromTwoMatricesOneSample(matrix1, tail, tThreshold, zThreshold)
% extractClusterFromTwoMatricesOneSample performs point-by-point
% statistical comparison and extract clusters of significant time points by
% comparing subtracted conditions to a null distribution.  
%
% [Y1, Y2, Y3, Y4, Y5, Y6, Y7, Y8, Y9] = extractClusterFromTwoMatricesOneSample(X1, X2, X3, X4, X5, X6)
...
% X2 : two-tailed or one-tailed test
% X3 : threshold for t-values to define clusters (parametric)
% X4 : threshold for z-values to define clusters (non-parametric)
% 
...
[numberOfPoints numberOfSubject1]=size(matrix1);
    ...
    for cptTimepoint=1:numberOfPoints
        ...
        if ~ (isempty(distributionAtChosenPoint1))
